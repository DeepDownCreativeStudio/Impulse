...
+ (GameData *)loadData { 
    ...
    for (GDataXMLElement *element in dataArray) {
        ...
        NSArray *tiltArray = [element elementsForName:@"Tilt"];        
        ...
        // tilt
        if (tiltArray.count > 0) {
            GDataXMLElement *tiltElement = (GDataXMLElement *) [tiltArray objectAtIndex:0];
            tilt = [[tiltElement stringValue] boolValue];
        }
...
+ (void)saveData:(GameData *)saveData { 
    ...
    GDataXMLElement *tiltElement = 
    [GDataXMLNode elementWithName:@"Tilt"   
                      stringValue:[[NSNumber numberWithBool:saveData.tilt] stringValue]];
    ...
    [gameDataElement addChild:tiltElement];
